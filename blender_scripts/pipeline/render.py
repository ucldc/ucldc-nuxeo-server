...
bpy.ops.object.camera_add(enter_editmode=False, location=cam_location)
...
bpy.context.collection.objects.link(sun_front)
...
bpy.context.collection.objects.link(sun_back)
...
bpy.context.collection.objects.link(sun_left)
...
bpy.context.collection.objects.link(sun_right)
...
for obj in scene.objects:
    ...
    if obj.type in ['CAMERA', 'LAMP', 'EMPTY']:
        obj.select_set(True)
